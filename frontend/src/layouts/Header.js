...
export default function Header({ onSkin }) {
  ...
  const [isLoggingOut, setIsLoggingOut] = useState(false); // To manage logout state
  ...
  const handleLogout = async () => {
    try {
      ...
      // Set logging out state to true to prevent multiple clicks
      ...
      // Clear the token from localStorage (immediately after the request)
      ...
      // Clear the token if logout fails, to ensure no invalid session remains
      localStorage.removeItem("authToken");
      ...
      // Reset logging out state after the logout process is complete
